...
void star_operation(t_blif_cube **result_cover, t_blif_cube *A, t_blif_cube *B, int input_count){
	...
	//printf("%d ", read_cube_variable(A->signal_status, 0));
	//printf("%d ", read_cube_variable(A->signal_status, 1));
	//printf("%d ", read_cube_variable(A->signal_status, 2));
	//printf("%d **** ", read_cube_variable(A->signal_status, 3));
	//printf("%d ", read_cube_variable(B->signal_status, 0));
	//printf("%d ", read_cube_variable(B->signal_status, 1));
	//printf("%d ", read_cube_variable(B->signal_status, 2));
	//printf("%d\n ", read_cube_variable(B->signal_status, 3));
	//printf("\n");
	t_blif_cube *result_cube = malloc(sizeof(t_blif_cube));
	...
	if (count <= 1){
		...
		printf("%d ", read_cube_variable(result_cover[0]->signal_status, 0));
		printf("%d ", read_cube_variable(result_cover[0]->signal_status, 1));
		printf("%d ", read_cube_variable(result_cover[0]->signal_status, 2));
		printf("%d \n ", read_cube_variable(result_cover[0]->signal_status, 3));
 ...
 */
{
	...
	// find G cover
	t_blif_cube **G_cover = (t_blif_cube **) malloc(((f->cube_count - 1) * f->cube_count) / 2 * sizeof (t_blif_cube *));
	printf("From * op: \n");
	for (int i = 0; i < f->cube_count - 1; i++){
		for (int j = i + 1; j < f->cube_count; j++){
			star_operation(G_cover, f->set_of_cubes[i], f->set_of_cubes[j], f->input_count);
		}
	}
	// new cover C union G
	t_blif_cube **union_cover = (t_blif_cube **)malloc((((f->cube_count - 1) * f->cube_count) / 2 + f->cube_count)* sizeof (t_blif_cube *));
	// check PIs and generate {DC-set}
...
int main(int argc, char* argv[])
{
	...
	printf("'* Operation' and '# Operation' 2-level logic minimization program.\r\n");
